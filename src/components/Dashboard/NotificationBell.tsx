...
export function NotificationBell({ id }: { id?: string }) {
  ...
  return (
    <div className="relative">
      ...
      >
        <Bell className={cn("w-6 h-6 transition-colors", unreadCount > 0 ? "text-primary" : "text-slate-400 group-hover:text-primary")} />
        {unreadCount > 0 && (
          <span className="absolute top-2 right-2 w-4 h-4 bg-primary text-white text-[10px] font-black rounded-full flex items-center justify-center border-2 border-white animate-in zoom-in duration-300">
            {unreadCount}
          </span>
        )}
      </button>
      ...
      <AnimatePresence>
        {isOpen && (
          <>
            ...
            >
              <div className="p-6 border-b border-theme-background flex items-center justify-between bg-theme-background/50">
                      ...
                      >
                        <div className="flex gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs font-medium text-theme-background0 line-clamp-2 mt-0.5">
                              {n.message}
                            </p>
              ...
              {notifications.length > 0 && (
                <div className="p-4 bg-theme-background/50 border-t border-theme-background text-center">
