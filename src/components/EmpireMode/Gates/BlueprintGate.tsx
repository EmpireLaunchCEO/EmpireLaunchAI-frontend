    ...
    <div className="space-y-8">
      <div className="flex items-center gap-4 bg-primary/5 p-6 rounded-[32px] border border-primary/10">
        ...
        <div>
          ...
          <p className="text-sm text-theme-background0 font-medium italic">"I've found a high-growth niche in {payload?.niche ?? 'your business sector'}. Proceed with this plan?"</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-theme-surface border-2 border-theme-background rounded-[40px] p-8 space-y-6 shadow-sm">
          ...
          <div className="space-y-4">
                ...
                {isEditing ? (
                  <div className="flex items-center gap-2">
                    ...
                    <input 
                      ...
                      className="w-20 bg-theme-surface border border-theme rounded-lg px-2 py-1 text-sm font-bold focus:outline-none focus:border-primary"
                    />
                  </div>
                ) : (
            ...
            <button className="w-full py-3 border-2 border-dashed border-theme rounded-2xl flex items-center justify-center gap-2 text-slate-400 hover:border-blue-200 hover:text-primary transition-all">
          ...
          <div className="space-y-6 relative z-10 border-l-2 border-white/5 ml-2">
             {(editedPayload?.roadmap ?? []).map((item, i) => (
               <div key={i} className="relative pl-6">
                  <div className="absolute left-[-9px] top-1.5 w-4 h-4 rounded-full bg-slate-900 border-2 border-primary" />
                  <div className="space-y-1">
                     <span className="text-[10px] font-black text-primary uppercase tracking-widest">Day {item.day}</span>
      ...
      <div className="flex gap-4">
        ...
        <button 
          ...
          className="flex-[2] py-5 bg-primary text-white rounded-3xl font-black text-xs uppercase tracking-[0.2em] hover:bg-slate-900 transition-all shadow-xl shadow-blue-200 flex items-center justify-center gap-2 group"
        >
          Activate Strategy
